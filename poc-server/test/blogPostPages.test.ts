...
describe('blog post list page API wiring', () => {
  ...
  it('wires the Blog management V2 batch generation button and progress modal to poc-server APIs', () => {
    ...
    expect(blogManagement).toContain('id="blog-auto-generate-count"');
    expect(blogManagement).toContain('id="blog-auto-generate-count-input"');
    expect(blogManagement).toContain('type="number"');
    expect(blogManagement).toContain('min="1"');
    expect(blogManagement).toContain('max="10"');
    expect(blogManagement).not.toContain('성과 데이터 마지막 업데이트');
    expect(js).toContain('AUTO_GENERATION_INTERVAL_DAYS = 14');
    expect(js).toContain('AUTO_GENERATION_MIN_COUNT = 1');
    expect(js).toContain('AUTO_GENERATION_MAX_COUNT = 10');
    expect(js).toContain('currentAutoGenerationCount');
