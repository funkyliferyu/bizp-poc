...
describe('AI training onboarding page API wiring', () => {
  it('loads a page-specific training settings script without redesigning the page shell', () => {
    ...
    expect(html).toContain('id="training-youtube-url"');
    expect(html).toContain('id="training-tiktok-url"');
    ...
    expect(html).toContain('id="training-youtube-status"');
    expect(html).toContain('id="training-tiktok-status"');
  ...
  it('calls only poc-server training and collection APIs from browser code', () => {
    ...
    expect(js).toContain("sourceUrl: textValue('training-youtube-url')");
    expect(js).toContain("sourceUrl: textValue('training-tiktok-url')");
    expect(js).toContain('function deriveStoreChannelSources');
    expect(js).toContain('function setFutureProviderStatus');
    ...
    expect(js).toContain("channelSource(payload, 'youtube') ||");
    expect(js).toContain("channelSource(payload, 'tiktok') ||");
    expect(js).toContain('firstUrlMatching(homepageUrls, /(^|\\/\\/)(m\\.)?blog\\.naver\\.com\\//i)');
    expect(js).toContain('firstUrlMatching(homepageUrls, /(^|\\/\\/)(www\\.)?(youtube\\.com|youtu\\.be)\\//i)');
    expect(js).toContain('firstUrlMatching(homepageUrls, /(^|\\/\\/)(www\\.)?tiktok\\.com\\//i)');
    expect(js).toContain("setTextValue('training-blog-url', settings.channels.naverBlog.sourceUrl || storeChannelSources.blog)");
    expect(js).toContain("setTextValue('training-youtube-url', settings.channels.youtube.sourceUrl || storeChannelSources.youtube)");
    expect(js).toContain("setTextValue('training-tiktok-url', settings.channels.tiktok.sourceUrl || storeChannelSources.tiktok)");
